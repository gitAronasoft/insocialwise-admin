...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('subscription_plans', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('subscription_plans', 'fb_ads_analytics')) {
                $table->boolean('fb_ads_analytics')->default(false)->after('white_label');
            }
            if (!Schema::hasColumn('subscription_plans', 'fb_ads_creation')) {
                $table->boolean('fb_ads_creation')->default(false)->after('fb_ads_analytics');
            }
            if (!Schema::hasColumn('subscription_plans', 'team_roles_permissions')) {
                $table->boolean('team_roles_permissions')->default(false)->after('fb_ads_creation');
            }
            if (!Schema::hasColumn('subscription_plans', 'client_workspaces')) {
                $table->boolean('client_workspaces')->default(false)->after('team_roles_permissions');
            }
            if (!Schema::hasColumn('subscription_plans', 'display_features')) {
    ...
    public function down(): void
    {
        Schema::table('subscription_plans', function (Blueprint $table) {
            $columns = [
                'slug', 'monthly_price_usd', 'yearly_price_usd', 'monthly_price_inr', 'yearly_price_inr',
                'ai_tokens_per_month', 'ai_auto_comment_reply', 'ai_auto_dm_reply', 'ai_semantic_analysis',
                'ai_driven_reporting', 'ai_content_generator', 'social_profile_score', 'calendar_scheduling',
                'unified_inbox', 'export_reports', 'white_label', 'fb_ads_analytics', 'fb_ads_creation',
                'team_roles_permissions', 'client_workspaces', 'display_features', 'support_level',
            ];
