...
<div class="space-y-6" x-data="webhookForm()">
    <x-breadcrumb :items="[
        ['label' => 'Webhooks', 'url' => route('admin.webhooks.index')], ['label' => 'Edit', 'url' => null]
    ]" />
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Update webhook configuration for {{ $webhook->name }}</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
        <form action="{{ route('admin.webhooks.update', $webhook) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="provider" id="provider" required
                        ...
                        <option value="stripe" {{ old('provider', $webhook->provider) == 'stripe' ? 'selected' : '' }}>Stripe</option>
                        <option value="facebook" {{ old('provider', $webhook->provider) == 'facebook' ? 'selected' : '' }}>Facebook</option>
                        <option value="linkedin" {{ old('provider', $webhook->provider) == 'linkedin' ? 'selected' : '' }}>LinkedIn</option>
                        <option value="n8n" {{ old('provider', $webhook->provider) == 'n8n' ? 'selected' : '' }}>n8n</option>
                        <option value="custom" {{ old('provider', $webhook->provider) == 'custom' ? 'selected' : '' }}>Custom</option>
                    </select>
                    @error('provider')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="event_type" id="event_type" required
                        ...
                        <template x-for="(label, value) in eventTypes[selectedProvider] || {}" :key="value">
                            <option :value="value" x-text="label" :selected="value === selectedEventType"></option>
                        </template>
                    </select>
                    @error('event_type')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="url" name="endpoint_url" id="endpoint_url" value="{{ old('endpoint_url', $webhook->endpoint_url) }}" required
                        class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                <div>
                    <label for="secret" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        Webhook Secret
                        <span class="text-gray-400 font-normal">(Leave empty to keep current secret)</span>
                    </label>
                    <div class="mt-1 relative">
                        <input :type="showSecret ? 'text' : 'password'" name="secret" id="secret"
                    ...
                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        Current secret: <span class="font-mono">{{ $webhook->masked_secret }}</span>
                    </p>
                </div>
            </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-4">
                <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Webhook Statistics</h4>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Created:</span>
                        <p class="text-gray-900 dark:text-white">{{ $webhook->created_at->format('M d, Y H:i') }}</p>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Last Triggered:</span>
                        <p class="text-gray-900 dark:text-white">{{ $webhook->last_triggered_at ? $webhook->last_triggered_at->format('M d, Y H:i') : 'Never' }}</p>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Success Count:</span>
                        <p class="text-green-600 dark:text-green-400">{{ $webhook->success_count ?? 0 }}</p>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Failure Count:</span>
                        <p class="text-red-600 dark:text-red-400">{{ $webhook->failure_count ?? 0 }}</p>
                    </div>
                </div>
            </div>
...
function webhookForm() {
    return {
        selectedProvider: '{{ old('provider', $webhook->provider) }}',
        selectedEventType: '{{ old('event_type', $webhook->event_type) }}',
        showSecret: false,
