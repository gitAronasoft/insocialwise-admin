...
<div class="space-y-6">
    <x-breadcrumb :items="[
        ['label' => 'Pages', 'url' => route('admin.pages.index')], ['label' => 'View Details', 'url' => null]
    ]" />
    <div class="flex justify-between items-center">
        <a href="{{ route('admin.pages.index') }}" class="text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300">
            &larr; Back to Pages
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
            ...
            <div class="mt-6 space-y-3">
                ...
                <div class="flex justify-between text-sm">
                    ...
                    @else
                        <span class="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400">Inactive</span>
                    @endif
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-gray-500 dark:text-gray-400">Connected</span>
                    <span class="text-gray-900 dark:text-gray-100">{{ $page->createdAt ? $page->createdAt->format('M d, Y') : 'N/A' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <div class="flex items-center space-x-3">
                            <div class="w-12 h-12 rounded-full bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center">
                                <span class="text-primary-600 dark:text-primary-400 font-bold">{{ strtoupper(substr($page->socialUser->customer->firstName, 0, 1)) }}</span>
                            </div>
                            <div>
                                <p class="font-medium text-gray-900 dark:text-white">{{ $page->socialUser->customer->firstName }} {{ $page->socialUser->customer->lastName }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $page->socialUser->customer->email }}</p>
                            </div>
                        </div>
                        <a href="{{ route('admin.customers.show', $page->socialUser->customer->uuid) }}" 
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Access Token Status</h4>
                @if($page->socialUser)
                    @php
                        $tokenExpiry = $page->socialUser->token_expires_at;
                    ...
                    @endif
                @else
                    <p class="text-gray-500 dark:text-gray-400 text-center py-4">Token information not available</p>
                @endif
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Connected Social Account</h4>
                @if($page->socialUser)
                    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <div class="flex items-center space-x-3">
                            @if($page->socialUser->profile_pic)
                                <img src="{{ $page->socialUser->profile_pic }}" alt="{{ $page->socialUser->name }}" class="w-10 h-10 rounded-full">
                            @else
                                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                                    <span class="text-blue-600 dark:text-blue-400 font-bold">{{ strtoupper(substr($page->socialUser->name ?? 'S', 0, 1)) }}</span>
                                </div>
                            @endif
                            <div>
                                <p class="font-medium text-gray-900 dark:text-white">{{ $page->socialUser->name }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $page->socialUser->email ?? 'No email' }}</p>
                            </div>
                        </div>
                        <a href="{{ route('admin.social-accounts.show', $page->socialUser) }}" 
                           class="text-primary-600 dark:text-primary-400 hover:text-primary-800 dark:hover:text-primary-300 text-sm font-medium">
                            View Account
                        </a>
                    </div>
                @else
                    <p class="text-gray-500 dark:text-gray-400 text-center py-4">No social account linked</p>
                @endif
            </div>
        </div>
    </div>
